...
const createMap = () =>{
    for (let row = 0; row<12; row++) {
        for(let column=0; column<10; column++){
            if(grid[row][column]){ //si no esta vacio
...
const gravityFall = (row, column, blockColor)=>{ //agregar un dalay en la caida
    if(!(grid[row+1][column])){
        ...
        setTimeout(e=>{gravityFall(++row,column,blockColor)},200);
    }else{
        checkMatch(row, column, blockColor);
    }
}
    ...
    if(!(grid[row][column+i])){
        ...
        for (let prevrow=row; prevrow > 1; prevrow--) {
            if(grid[prevrow-1][column] && grid[prevrow-1][column]!=9){
                setTimeout(e=>{gravityFall(prevrow-1,column,grid[prevrow-1][column])},200);
            }
        }
        column +=i;
        selector.column +=i;
        setTimeout(e=>{gravityFall(row,column,grid[row][column])},200);
...
const checkMatch = (row, column, colorBlock) =>{
    if((grid[row+1][column]==colorBlock) || (grid[row][column+1]==colorBlock) || (grid[row][column-1]==colorBlock)){
        if(grid[row+1][column]==colorBlock) deleteBlock(row+1,column);                    
        if(grid[row][column+1]==colorBlock) deleteBlock(row,column+1);
        if(grid[row][column-1]==colorBlock) deleteBlock(row,column-1);
...
const move = direction =>{
    if(selector.selected) checkCollision(selector.row,selector.column,direction);
    else selector.column +=(direction=="right")?1:-1;
}

document.addEventListener("keydown", e=>{
    switch (e.key) {
        case "ArrowUp": if(selector.row>0 && !selector.selected)selector.row -=1; break;
        case "ArrowDown": if(selector.row<11 && !selector.selected)selector.row +=1;break;
        case "ArrowRight": if(selector.column<9) move("right");break
        case "ArrowLeft": if(selector.column>0) move("left");break
